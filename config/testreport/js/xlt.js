(function($){
    ...
    function recalculateFilteredFooter(table, filteredFooterRow, staticFooterRow) {
        ...
        function determineColumnDescriptions() {
            ...
            function create2DimensionalArrayWithEmptyRows(numberOfRows) {
              var result = [];
        ...
        function buildTotalsCellForColumn(columnIndex, columnDescription, displayedRows, staticFooterCell) {
            ...
            function determineTotalsFunctionBy(columnDescription) {
                ...
                var func;
                if (regexForColumnsWithSumTotal.test(columnDescription)) {
                    func = function(array) { return array.reduce(sum, 0); };
                }
                else if (regexForColumnsWithMinTotal.test(columnDescription)) {
                    func = function(array) { return array.reduce(minimum, Number.POSITIVE_INFINITY); };
                }
                else if (regexForColumnsWithMaxTotal.test(columnDescription)) {
                    func = function(array) { return array.reduce(maximum, Number.NEGATIVE_INFINITY); };
                }
                else if (regexForColumnsWithAverageTotal.test(columnDescription)) {
            ...
            var totalsFunction = determineTotalsFunctionBy(columnDescription);
            ...
            displayedRows.each( function() {
                ...
                var cellTextWithoutCommasAndSpaces = cell.text().replace( /,|\s/g , '');
                displayedColumnValues.push(Number(cellTextWithoutCommasAndSpaces));
            ...
            var totalsValue = totalsFunction(displayedColumnValues).toFixed(maxDecimalPlaces);
        ...
        for (var columnIndex = 1; columnIndex < numberOfColumns; ++columnIndex) {
            filteredFooterRowContents +=
                buildTotalsCellForColumn(columnIndex, columnDescriptions[columnIndex], displayedRows, staticFooterCells.eq(columnIndex));
        }
